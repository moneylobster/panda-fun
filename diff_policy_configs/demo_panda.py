"""
Usage:
(robodiff)$ python demo_panda.py -o <demo_save_dir> -d <device>
"""
...
def main(output, robot_ip, vis_camera_idx, init_joints, frequency, command_latency, device):
            ...
            with teleop_class(currentpose) as teleop:
                ...
                while not stop:
                    ...
                    if teleop.endevent.is_set():
                        ...
                        stop = True
                    elif teleop.startevent.is_set():
                        # Start recording
                        teleop.startevent.clear()
                        env.start_episode(t_start + (iter_idx + 2) * dt - time.monotonic() + time.time())
                        is_recording = True
                        print('Recording!')
                    elif teleop.stopevent.is_set():
                        # Stop recording
                        teleop.stopevent.clear()
                        env.end_episode()
                        is_recording = False
                        print('Stopped.')

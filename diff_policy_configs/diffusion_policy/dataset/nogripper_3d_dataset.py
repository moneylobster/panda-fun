...
class Nogripper3dDataset(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    if action_shape == (9,):
        ...
        zarr_resize_index_last_dim(zarr_arr, idxs=[0,1,2,3,4,5,6,7,8]) # For 9D action representations!!
    
    for key, shape in lowdim_shapes.items():
        if 'pose' in key and shape == (9,):
            zarr_arr = replay_buffer[key]
            zarr_resize_index_last_dim(zarr_arr, idxs=[12,13,14,
                                                       0,1,2,
                                                       3,4,5]) # For 16D pose representations

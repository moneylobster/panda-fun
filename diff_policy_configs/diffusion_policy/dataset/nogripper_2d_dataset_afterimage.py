...
class Nogripper2dDatasetAfterimage(BaseImageDataset):
    def __init__(self,
            shape_meta: dict,
            dataset_path: str,
            horizon=1,
...
def _get_replay_buffer(dataset_path, shape_meta, store):
    ...
    for key, attr in obs_shape_meta.items():
        ...
        if type == 'rgb':
            ...
            if 'pose' in key:
                assert tuple(shape) in [(2,),(6,),(9,)]
    
    action_shape = tuple(shape_meta['action']['shape'])
    assert action_shape in [(2,),(6,),(9,)]
    ...
    # TODO need to add something somewhere here i think

    # transform lowdim dimensions
    if action_shape == (2,):
        # 2D action space, only controls X and Y
        zarr_arr = replay_buffer['action']
        zarr_resize_index_last_dim(zarr_arr, idxs=[0,1]) # For 9D action representations!!
    
    for key, shape in lowdim_shapes.items():
        if 'pose' in key and shape == (2,):
            # only take X and Y
            zarr_arr = replay_buffer[key]
            zarr_resize_index_last_dim(zarr_arr, idxs=[12,13]) # For 16D pose representations
